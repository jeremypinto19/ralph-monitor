...
- **`components/`** — UI and feature components.
  ...
  - **`components/dashboard-shell.tsx`** — dashboard layout wrapper: fixed **`Sidebar`** from `md` up; below `md`, a top bar + **Sheet** (shadcn `components/ui/sheet.tsx`) exposes the same **`SidebarNav`** as the rail.
- **`lib/`** — Shared logic: **`dynamo.ts`** (AWS SSM → STS → DynamoDB), **`posthog.ts`** (server HogQL), **`conversation-posthog-bundle.ts`** (batched HogQL journey + checkout enrich pipeline for the conversations dashboard), **`conversations-api-cache.ts`** (Next.js `unstable_cache` for `/api/conversations` shop names and HogQL list batches; full Dynamo scan is not Data Cached — Next.js caps entries at ~2MB), **`posthog-row-cache.ts`** (short-lived in-memory bridge between GET `/api/conversations` and POST `posthog-bundle` to avoid duplicate journey HogQL), **`conversation-feedback.ts`** (assistant message vote/reasons summary + Dynamo feedback parsing for the API), **`checkout-filters.ts`** (Shopify vs JUST × checkout stage matching for Checkout + Conversations client filters), **`types.ts`** (domain types aligned with backend Pydantic), **`utils.ts`** (`cn()`, etc.).
