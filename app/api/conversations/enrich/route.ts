...
async function resolveDistinctIdByConversationId(
  ...
  const result = await queryPostHog(`
    ...
    WHERE event = 'just_ai_session_started'
      AND JSONExtractString(properties, 'conversationId') = '${conversationId.replace(/'/g, "\\'")}'
...
export async function POST(request: Request) {
  try {
    ...
    const resolved = new Map<
      string,
      { distinctId: string; key: ConversationKey }
    >();
    const batchSize = 10;
    for (let i = 0; i < conversations.length; i += batchSize) {
      ...
      const results = await Promise.all(
        batch.map(async (c) => {
          const byCid = await resolveDistinctIdByConversationId(
            c.conversationId,
          );
          const did =
            byCid ??
            (c.shopId && c.startedAt
              ? await resolveDistinctId(c.shopId, c.startedAt)
              : null);
          if (did)
            return {
    ...
    const allDids = [
      ...new Set([...resolved.values()].map((v) => v.distinctId)),
    ];
    const inClause = allDids
      .map((d) => `'${d.replace(/'/g, "\\'")}'`)
      .join(",");
